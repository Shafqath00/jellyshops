...
export function StorefrontRenderer({ document, page = "home", pageId, mode, commerce, selected, onSelect, onSectionError }: StorefrontRendererProps) {
  if (isStorefrontDocument(document)) {
    ...
    const renderRegion = (region: "header" | "footer") => document.regions[region].map((section) => <RegistrySectionRenderer key={section.id} section={section} mode={mode} commerce={commerce} region={region} selected={selected} onSelect={onSelect} />);
    return <div className="jelly-storefront" data-jelly-theme={document.theme.presetId} style={style}>{renderRegion("header")}<main>{activeSections.map((section) => <RegistrySectionRenderer key={section.id} section={section} mode={mode} commerce={commerce} region="template" selected={selected} onSelect={onSelect} />)}</main>{renderRegion("footer")}</div>;
  }
