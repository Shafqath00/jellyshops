...
export function StorefrontRenderer({ document, page = "home", pageId, mode, commerce, selected, onSelect, onSectionError }: StorefrontRendererProps) {
  if (isStorefrontDocument(document)) {
    ...
    const renderRegion = (region: "header" | "footer") => document.regions[region].map((section) => <SectionRenderer key={section.id} section={section} mode={mode} commerce={commerce} region={region} selected={selected} onSelect={onSelect} />);
    return <div className="jelly-storefront" data-jelly-theme={document.theme.presetId} style={style}>{renderRegion("header")}<main>{activeSections.map((section) => <SectionRenderer key={section.id} section={section} mode={mode} commerce={commerce} region="template" selected={selected} onSelect={onSelect} />)}</main>{renderRegion("footer")}</div>;
  }
  const style = resolveDesignTokens(document.theme.id, document.globalSettings) as CSSProperties;
  return <div className="jelly-storefront" data-jelly-theme={document.theme.id} style={style}><SectionRenderer section={document.header} mode={mode} commerce={commerce} /><main><PageRenderer page={document.pages[page]} mode={mode} commerce={commerce} onSectionError={onSectionError} /></main><SectionRenderer section={document.footer} mode={mode} commerce={commerce} /></div>;
}
