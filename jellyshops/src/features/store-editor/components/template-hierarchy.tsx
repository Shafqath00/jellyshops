...
export function TemplateHierarchy({
  ...
  globalSections: Record<string, { id: string; name: string }>;
    ...
    <aside className="flex h-full min-h-0 flex-col bg-white" aria-label="Template hierarchy">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        ...
        <div className="space-y-1">
          {templatePlacements.map((placement, index) => {
          ...
          {templatePlacements.length === 0 && (
            <p className="px-2 py-6 text-center text-[12px] text-[#8c9196]">No sections in this template.</p>
          )}
        </div>
      </div>
    </aside>
