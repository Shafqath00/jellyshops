...
type MetafieldDefinitionRow = {
  id: string; storeId: string; ownerType: string; namespace: string; key: string;
  name: string; description: string | null; type: string; validations: unknown;
  storefrontVisible: boolean; origin: string; archivedAt: Date | null; createdAt: Date; updatedAt: Date;
};
type MetafieldValueRow = {
  id: string; storeId: string; definitionId: string; ownerId: string; value: unknown; updatedAt: Date;
};
type MetaobjectDefinitionRow = {
  id: string; storeId: string; handle: string; name: string; storefrontVisible: boolean;
  fields: unknown; archivedAt: Date | null; createdAt: Date; updatedAt: Date;
};
type MetaobjectEntryRow = {
  id: string; storeId: string; definitionId: string; handle: string; displayName: string;
  values: unknown; archivedAt: Date | null; createdAt: Date; updatedAt: Date;
};

function mapDefinition(record: MetafieldDefinitionRow): MetafieldDefinitionRecord {
  return {
    id: record.id, storeId: record.storeId, ownerType: record.ownerType as CustomDataOwnerType,
    namespace: record.namespace, key: record.key, name: record.name,
    ...(record.description !== null ? { description: record.description } : {}),
    type: record.type as DynamicValueType,
    ...(record.validations && typeof record.validations === "object" ? { validations: record.validations as Record<string, unknown> } : {}),
...
function mapValue(record: MetafieldValueRow): MetafieldValueRecord {
  return { id: record.id, storeId: record.storeId, definitionId: record.definitionId, ownerId: record.ownerId, value: record.value, updatedAt: record.updatedAt };
}

function mapMetaobjectDefinition(record: MetaobjectDefinitionRow): MetaobjectDefinitionRecord {
  if (!Array.isArray(record.fields)) throw new Error("Stored metaobject fields are invalid");
  return {
    id: record.id, storeId: record.storeId, handle: record.handle, name: record.name,
    storefrontVisible: record.storefrontVisible,
...
function mapMetaobjectEntry(record: MetaobjectEntryRow): MetaobjectEntryRecord {
  if (!record.values || typeof record.values !== "object" || Array.isArray(record.values)) {
    throw new Error("Stored metaobject values are invalid");
  }
  return {
    id: record.id, storeId: record.storeId, definitionId: record.definitionId, handle: record.handle,
    displayName: record.displayName, values: record.values as Record<string, unknown>,
    archivedAt: record.archivedAt, createdAt: record.createdAt, updatedAt: record.updatedAt,
  };
}
...
export class PrismaCustomDataRepository implements CustomDataRepository, MetaobjectRepository {
  ...
  async createMetafieldDefinition(input: CreateMetafieldDefinitionInput): Promise<MetafieldDefinitionRecord> {
    return mapDefinition(await this.client.metafieldDefinition.create({ data: { ...input, ...(input.validations ? { validations: input.validations as Prisma.InputJsonValue } : {}) } }));
  }
  ...
  async listMetafieldDefinitions(storeId: string, ownerType: CustomDataOwnerType): Promise<MetafieldDefinitionRecord[]> {
    return (await this.client.metafieldDefinition.findMany({ where: { storeId, ownerType }, orderBy: [{ namespace: "asc" }, { key: "asc" }] })).map(mapDefinition);
  }
  async updateMetafieldDefinition(storeId: string, id: string, patch: UpdateMetafieldDefinitionInput): Promise<MetafieldDefinitionRecord | null> {
    ...
    return mapDefinition(await this.client.metafieldDefinition.update({ where: { id }, data: {
  ...
  async setMetafieldValue(input: Omit<MetafieldValueRecord, "id" | "updatedAt">): Promise<MetafieldValueRecord> {
    return mapValue(await this.client.metafieldValue.upsert({
      ...
      create: { ...input, value: input.value as Prisma.InputJsonValue }, update: { value: input.value as Prisma.InputJsonValue },
    }));
  }
  async getMetafieldValue(storeId: string, definitionId: string, ownerId: string): Promise<MetafieldValueRecord | null> {
    const record = await this.client.metafieldValue.findUnique({ where: { storeId_definitionId_ownerId: { storeId, definitionId, ownerId } } });
  ...
  async ownerExists(storeId: string, ownerType: CustomDataOwnerType, ownerId: string): Promise<boolean> {
    switch (ownerType) {
      ...
      case "page": case "blog": case "article": return false;
    }
  }

  async createMetaobjectDefinition(input: CreateMetaobjectDefinitionInput): Promise<MetaobjectDefinitionRecord> {
    return mapMetaobjectDefinition(await this.client.metaobjectDefinition.create({ data: { ...input, fields: input.fields as unknown as Prisma.InputJsonValue } }));
  }
  ...
  async updateMetaobjectDefinition(storeId: string, id: string, patch: UpdateMetaobjectDefinitionInput): Promise<MetaobjectDefinitionRecord | null> {
    ...
    return mapMetaobjectDefinition(await this.client.metaobjectDefinition.update({ where: { id }, data: {
      ...(patch.name !== undefined ? { name: patch.name } : {}),
      ...(patch.storefrontVisible !== undefined ? { storefrontVisible: patch.storefrontVisible } : {}),
  ...
  async createMetaobjectEntry(input: CreateMetaobjectEntryInput): Promise<MetaobjectEntryRecord> {
    return mapMetaobjectEntry(await this.client.metaobjectEntry.create({ data: { ...input, values: input.values as Prisma.InputJsonValue } }));
  }
  ...
  async updateMetaobjectEntry(storeId: string, id: string, patch: UpdateMetaobjectEntryInput): Promise<MetaobjectEntryRecord | null> {
    ...
    return mapMetaobjectEntry(await this.client.metaobjectEntry.update({ where: { id }, data: {
  ...
  async listMetaobjectEntries(storeId: string, definitionId: string): Promise<MetaobjectEntryRecord[]> {
    return (await this.client.metaobjectEntry.findMany({ where: { storeId, definitionId }, orderBy: { handle: "asc" } })).map(mapMetaobjectEntry);
  }
}
