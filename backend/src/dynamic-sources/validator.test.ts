...
function repositories() {
  const customData: Pick<CustomDataRepository, "listMetafieldDefinitions"> = {
    listMetafieldDefinitions: async (_storeId, ownerType) => ownerType === "product" ? [
      { id: "material-def", storeId: "store-a", ownerType: "product", namespace: "details", key: "material", name: "Material", type: "string", storefrontVisible: true, origin: "jelly", archivedAt: null, createdAt: new Date(0), updatedAt: new Date(0) },
      { id: "internal-def", storeId: "store-a", ownerType: "product", namespace: "internal", key: "supplier", name: "Supplier", type: "string", storefrontVisible: false, origin: "jelly", archivedAt: null, createdAt: new Date(0), updatedAt: new Date(0) },
      { id: "brand-def", storeId: "store-a", ownerType: "product", namespace: "details", key: "brand", name: "Brand", type: "metaobject_reference", storefrontVisible: true, origin: "jelly", validations: { metaobjectDefinitionId: "brand-type" }, archivedAt: null, createdAt: new Date(0), updatedAt: new Date(0) },
    ] : [],
  };
...
describe("dynamic source registry", () => {
  ...
  it("discovers compatible visible fields nested through metaobject references", async () => {
    const { customData, metaobjects } = repositories();
    const registry = new DynamicSourceRegistry(customData, metaobjects);
    const sources = await registry.listDynamicSources("store-a", "product", ["image"]);
    expect(sources).toEqual(expect.arrayContaining([
      expect.objectContaining({
        id: "metafield:product:details.brand.logo",
        valueType: "image",
        binding: {
    ...
    expect(sources.map(({ id }) => id)).not.toContain("metafield:product:details.brand.secret");
  });

  it("rejects incompatible source types", async () => {
    const { customData, metaobjects } = repositories();
    const registry = new DynamicSourceRegistry(customData, metaobjects);
    await expect(validateDynamicBinding(registry, "store-a", { kind: "resource_field", resource: "product", field: "vendor" }, "image", "product")).rejects.toThrow(/image/i);
  });

  it("rejects product-specific sources in home context", async () => {
    const { customData, metaobjects } = repositories();
    const registry = new DynamicSourceRegistry(customData, metaobjects);
    await expect(validateDynamicBinding(registry, "store-a", { kind: "resource_field", resource: "product", field: "title" }, "string", "home")).rejects.toThrow(/context/i);
  });
  ...
  it("resolves visible metaobject fields through a typed metafield reference", async () => {
    ...
    await expect(validateDynamicBinding(registry, "store-a", binding, "image", "product")).resolves.toMatchObject({ valueType: "image" });
    await expect(validateDynamicBinding(registry, "store-a", { ...binding, field: "secret" }, "string", "product")).rejects.toThrow(/visible|unavailable/i);
  });
});
