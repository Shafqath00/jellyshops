...
function requireTestDatabaseUrl(): string {
  ...
  if (!value) throw new Error("TEST_DATABASE_URL is required and must target a disposable local jellyshops_test database");
  ...
  if (!loopback || parsed.pathname !== "/jellyshops_test") {
    throw new Error("TEST_DATABASE_URL must target a disposable loopback database named jellyshops_test");
  }
  return value;
}
...
export async function createIntegrationDatabase(): Promise<IntegrationDatabase> {
  ...
  const administration = new Pool({ connectionString: databaseUrl, max: 1, connectionTimeoutMillis: 5_000, application_name: "jellyshops-integration-setup" });
  ...
  try {
    ...
    try {
      ...
      for (const migrationFile of migrationFiles) await migrationConnection.query(await migrationSql(migrationFile));
    } finally {
